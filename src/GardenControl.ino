...
void heartbeat() {
  ...
  if (heartbeat_currentMillis - heartbeat_previousMillis >= heartbeat_interval) {
    ...
    Serial.println("");
    client.publish("home/outdoor/cistern/heartbeat", "on");
  }
}
...
void setCisternStatus(char* topic, byte* payload, unsigned int length) {
  ...
  if (mqttTopic == "home/outdoor/cistern/pump")
  {
    if (mqttPayload == "on") {
      ...
      Serial.print(" is ");
      Serial.println(pinStatus);
      client.publish("home/outdoor/cistern/pump/response", "on");
      delay(1000);
      ...
      Serial.print(" is ");
      Serial.println(pinStatus);
      client.publish("home/outdoor/cistern/pump/response", "off");
      delay(1000);
      emergencystop_running = false;
      emergencystop_running_prev = false;
    } else {
  ...
  else if (mqttTopic == "home/outdoor/cistern/socket")
  {
    if (mqttPayload == "on") {
      ...
      Serial.print(" is ");
      Serial.println(pinStatus);
      client.publish("home/outdoor/cistern/socket/response", "on");
      delay(1000);
    } else if (mqttPayload == "off") {
      ...
      Serial.print(" is ");
      Serial.println(pinStatus);
      client.publish("home/outdoor/cistern/socket/response", "off");
      delay(1000);
    } else {
      Serial.println("No valid mqtt command");
    }
